...
NRsurrogate models, which interpolate directly over waveforms produced by numerical relativity (NR) simulations, are in principle also straightfoward to implement in \jax and will be explored in the future.
...
Firstly, we illustrate how the coefficients that form part of the IMRPhenomD waveform model could be improved by high dimensional fitting enabled by an automatically differentiable waveform. 
...
Again, performing the same benchmark as above on an NVIDIA A100 with 40GB of memory, we find that waveform evaluations are over an order of magnitude faster than serial CPU evaluation.
...
One of the primary aims of this paper is to show that waveform derivatives will also be highly valuable to data analysis tasks. 
AD provides two big advantages when it comes to evaluating derivatives compared to numerical differentiation.
First, the accuracy of derivatives from AD is significantly more stable than finite-difference methods.
...
Having an accurate waveform model is essential for many data analysis tasks throughout GW science.
...
We therefore aim to improve the accuracy of the waveform by jointly fitting all coefficients at once.\footnote{
    For a further breakdown of how the optimization changes across the parameter space, see our accompanying paper~y\citep{Lam:2023oga}.
}
...
\begin{figure}[t!]
	...
	\begin{centering}
		...
		\caption{
            ...
            The blue line shows the relative error of the IMRPhenomD model with the original model coefficients where as the red line uses the IMRPhenomD model after optimization using gradient descent.
...
\begin{figure}[t!]
	...
	\begin{centering}
		...
		\caption{
			Distribution of 536 log mismatches (see Eq.~\eqref{eq:mismatch}) for the original (blue) and optimized (red) IMRPhenomD. 
...
We then run gradient descent, as described above, until the validation loss stops decreasing (the validation loss is calculated using the NR waveforms discussed below). 
...
In particular, we use 536 waveforms from the SXS catalog i.e., all waveforms with aligned spins, $|\chi_{x,y}|<5\times 10^{-3}$, and eccentricity $<2\times 10^{-3}$~\citep{Boyle:2019kee}.
...
In particular, the median mismatch (shown as vertical dashed lines in Fig.~\ref{fig:mismatch_hist}) is reduced by $\sim50$\%, indicating that our AD-assisted optimization procedure provides an improved implementation of the model.
...
Automatic derivatives, if implemented for EOB waveforms, could allow for the application of other sampling methods such as HMC, or possibly optimization over the entire set of NR waveforms at once.
...
\begin{figure}[t!]
    ...
    \caption{
        ...
        Automatic differentiation (AD) was used to compute the derivatives in Eq.~\eqref{eq:fisher}.
        As emphasized in the text, after JIT compilation, each error calculation was between three and five times faster than a numerical derivative forecasting code \texttt{GWbench}~\citep{Borhanian:2020ypi}.
    }
    \label{fig:chirp_mass_error}
\end{figure}

Forecasting the sensitivity of future experiments is a routine task in GW science. 
Due to its theoretical simplicity and evaluation speed, the Fisher matrix formalism (see e.g.,~\citep{Cornish:2010kf}) is commonly deployed to estimate how well a binary system's parameters could be measured.
...
\end{equation}
Note that here we have separated out the extrinsic ($\phi$) and intrinsic ($\Xi$) variables as well as introduced the detector projection functions for the plus and cross polarizations as $F^k_+$ and $F^k_\times$ respectively.
...
To illustrate the computational speed of computing Fisher matrices with AD, we consider a population of binaries and compute the $1\,\sigma$ chirp mass error.\footnote{
...
We have verified that our errors agree with a separate dedicated Fisher forecasting code~\citep{Borhanian:2020ypi} to within ~30\%.\footnote{
    Note that since Ref.~\citep{Borhanian:2020ypi} is based on numerical derivatives, the agreement is not expected to be close to machine precision.
}
...
After the search algorithms have constructed a list of confidently detected binaries, the next step is to sample from the posterior of each source parameter - so called PE.
...
Although we primarily discuss toy examples in this paper, each can be extended to the full data analysis task, some of which will be shown in upcoming papers~\citep{PEpaper, Lam:2023oga}.
...
T.E. would like to thank Luca Reali extensively for his help with running GWBench. This material is based upon work supported by NSF's LIGO Laboratory which is a major facility fully funded by the National Science Foundation.
