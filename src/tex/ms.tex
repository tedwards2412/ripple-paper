...
Due to its theoretical simplicity and its evaluation speed, the Fisher matrix formalism is commonly deployed to estimate how well the a particular binary system's parameters can be measure.
The Fisher matrix approach is built around the assumption of a Gaussian likelihood.
Although in practice this assumption is often violated in for real GW datasets, the results obtained using a Fisher analysis can provide quick and useful diagnotistics in evaluating sensitivities for a variety of models.

Computing the Fisher matrix requires one to evaluate derivatives of the likelihhood...
