...
\begin{abstract}
    Since their discovery in 2015, gravitational waves (GWs) have developed into a unique observational probe used throughout fundamental and astrophyics.
    As detections accumulate, data analysis task are becoming increasingly computationally demanding.
    ...
    Firstly, we demonstrate that highly accurate waveforms (here, IMRPhenomD) can be written in \jax and demonstrate that the serial evaluation speed of the waveform (and its derivative) is similar to the \lalsuite implementation in \CC.
    ...
    Firstly, we demonstrate how stochastic gradient descent can be used to optimize the $\sim 200$ coefficients that make up the waveform. \amc{what does ``make up the waveform'' mean?}
    In particular, we demonstrate that the typical match with numerical relativity waveforms can be improved by more than 10\% without any additional overhead.
    Secondly, we show that Fisher-forecasting calculations can be sped up by $\sim 100\times$ (on CPU) with no loss in accuracy.
    This increased speed makes population forecasting substantially simpler, as discussed extensively in \citep{Iacovelli:2022bbs, Iacovelli:2022mbg}. \amc{I'd personally avoid refs in the abstract}
...
% \section{Todo}
% \begin{itemize}
%     \item Finish intro \te{Done}
%     \item Finish section 2 \te{Done}
%     \item Write captions for all plots \te{Done}
...
In addition, this computational cost will substantially increase when next generation detectors \amc{should reference some here} come online.
...
In the most minimal example, this requires a Markov Chain Monte Carlo (MCMC) on an 11-dimensional parameter space for each event.
...
Finally, using the results of PE, population synthesis models constrain the progenitors systems from which the black holes we see merging today began their evolutionary journey~\citep{Wong:2022flg}.
All of these tasks require significant computing resources.
...
Derivatives are ubiqitously useful in data analysis tasks.
For instance, during PE, derivative information can be used to guide an optimizer towards higher-likelihood values (e.g.\ using gradient descent) or allow a sampler to rapidly explore parameter space (e.g.\ using Hamiltonian Monte Carlo~\citep{2011hmcm.book..113N,2017arXiv170102434B} or gradient descent). \amc{previously said HMC was exploring high-likelihood regions, which isn't quite right.}
Gradients are particularly helpful in high-dimensional spaces.
...
Here we make use of \jax~\citep{jax2018github} due to its easy integration with \texttt{python} libraries, seamless support for running code on different hardware accelerators (e.g.\ graphical processing units) and its just-in-time (JIT) compiler, which can substantially accelerate code. \amc{GPU support and JIT are separate features.}
...
In particular, their closed-form expressions and fixed-size frequency grids make a \jax implementation that complies with the constraints of JIT compilation simple to implement.
...
Secondly, we implement differentiable detector response functions and show that the speed of Fisher matrix calculations for approximate PE can be substantially improved \amc{accelerated?} using AD.
...
We have rewritten IMRPhenomD from scratch using a combination of pure \texttt{python} and \jax derivatives.
...
If we instead use a flat PSD in Eq.~\ref{eq:inner_prod} and scale the frequency grid correctly \amc{what do you mean by that?}, all dependence with $M$ seen in Fig.~\ref{fig:match} vanishes. 
...
Waveform calls can therefore be evaluated in parallel, massively improving performance. \amc{mention \texttt{vmap} and/or \texttt{pmap} -- super trivial to parallelize! More work in C.}
Performing the same benchmark as above on a Google Colab notebook with a freely-provided NVIDIA Tesla T4 GPU (far from the current state-of-the-art), we find that on average waveform evaluations take $\sim 0.00005\mathrm{ms}$, nearly five orders of magnitude faster.
...
First, derivatives from AD are significantly more stable than finite-difference methods.
...
are only approximations but can be evaluated much faster. \amc{redundant -- remove or merge with earlier material.}
...
\end{equation}
where $\alpha$ is the learning rate. We set $\alpha$ to be a very small number, i.e. $10^{-6}$, since the initial guess already lies close to the desired optimum. We terminate the program after 30000 steps, in which $\lambda$ has converged to the optimum. \amc{say something to explain why this is working. I.e.: does the loss initially drop and then plateau?}
...
waveforms with different intrinsic parameters implies there can be a trade-off in
performance in different regions of the intrinsic parameter space. \amc{this means your fit generally will depend on how the training waveforms are distributed over parameter space.}
...
Nevertheless, these do not present the whole picture; fast, general parameter estimation therefore remains a key aim of GW data analysis.
...
In particular, we will show that the autocorrelation time of a Hamiltonian Monte Carlo (HMC) sampler is significantly lower than a traditional MCMC algorithm.

For our basic example we perform an injection recovery test on a seven-dimensional parameter space with the same three detector setup considered in Sec.~\ref{subsec:fisher}.
...
Using a standard Gaussian likelihood, we then run the HMC sampler implemented in \flowMC for $2 \times 10^5$ steps and the gradient-free Gaussian random walk (GRW) sampler for $15 \times 10^5$ steps for comparison (each with four randomly-initialized independent chains). \amc{briefly summarize and give refs for flowMC and GRW. Is GRW Metropolis-Hastings with a Gaussian transition kernel? How'd you tune this? Why not use NUTS for HMC?}
The number of steps and and mass matrix used for each example was hand-tuned to give good performance for the specific sampler. \amc{not a huge deal, but why hand tune? There are automated ways to do this in e.g.\ numpyro with a warm-up adaptation phase.}
The additional steps for the GRW sampler were required to achieve a similarly converged posterior. \amc{Why not plot its posterior?}
...
In addition, we found that the effective number of samples \amc{provide ref?} (a measure of the number of indepedent samples) is between 2 and 7 times larger for HMC across the different dimensions of the parameter space, indicating its superior performance.
...
Ideally, future waveforms should be implemented either in pure python or using \jax directly. \amc{if they're pure python, some translation will be required due to jax's restrictions.}
