...
The entire optimized waveform distribution slightly shifted to the negative side. 
The peak of the original waveform distribution moved to the left end of the distribution. 
...
More specifically, we generate Gaussian noise consistent with the measured PSDs for each detector and then inject a BBH signal with parameters: $M_c = 23.82\,\mathrm{M_\odot}$,  $\eta = 0.248$, $\chi_1=0.3$, $\chi_1=-0.4$, $D = 1.6\,\mathrm{Gpc}$, $t_c = 0.0$, and $\phi_c = 0.0$.
Using a standard Gaussian likelihood, we then run the HMC sampler implemented in \flowMC for XX steps. 
\te{Describe more carefully what we do}

In Fig.~\ref{fig:corner}, the grey contours show the posterior recovered from our injected signal and the orange represents the injected parameters.
From the one dimensional histograms along the diagonal, it is clear that we successfully recover all seven parameters.
This is expected since the injected binary is relatively nearby.

To illustrate the benefit of a gradient based sampler, we run the same sampling as above but instead use a Gaussian random walk (GRW) sampler. 
