...
Benchmarking on a MacBook Pro with an $2.4$ GHz 8-Core Intel Core i9 processor, we find that a single waveform evaluation takes $\sim 1\,\mathrm{ms}$ for \lalsuite (interfaced with python) and $\sim 38\,\mathrm{s}$ for \ripple.\footnote{
    For this benchmark we used $32\,\mathrm{Hz}$ and $1024\,\mathrm{Hz}$ for the lowest and highest frequencies respectively. In addition we used a frequency spacing of $0.25\,\mathrm{Hz}$.
}
...
% Emphasise the stability of gradients.
% Not just a faster way of evaluating PhenomD.
% Change plot to be q vs chieff.
% Note that lalsuite cannot be made massively parallel on GPUs.
